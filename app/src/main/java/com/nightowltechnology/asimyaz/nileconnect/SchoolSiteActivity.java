...
public class SchoolSiteActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_school_site);
        Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar);
        setSupportActionBar(toolbar);
        ActionBar actionBar = getSupportActionBar();
        actionBar.setDisplayHomeAsUpEnabled(true);

        swipeView = (SwipeRefreshLayout)findViewById(R.id.swipe_school_site);
        web = (WebView) findViewById(R.id.webview_schooll);
        ...
        swipeView.setDistanceToTriggerSync(40);


        if(isNetworkConnected()) {
            web.loadUrl("http://www.nileuniversity.edu.ng");
        }
        else{

            web.setVisibility(View.GONE);
            AlertDialog.Builder builder = new AlertDialog.Builder(SchoolSiteActivity.this);
            builder.setMessage("sorry your network is not connected.");
            builder.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    Intent i = new Intent(SchoolSiteActivity.this,MainActivity.class);
                    startActivity(i);
                }
            });
